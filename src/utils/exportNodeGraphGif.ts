...
function snapColorsToPaletteLocal(palette: number[][], colors: number[][], threshold = 42) {
...
function drawFrame(context: CanvasRenderingContext2D, nodes: NodePoint[], settings: NodeGraphSettings, colors: EffectColorSettings, width: number, height: number, viewScale = 1) {
  ...
  if (settings.showConnections) {
    ...
    for (let i = 0; i < nodes.length; i += 1) {
      ...
      for (let j = i + 1; j < nodes.length; j += 1) {
        ...
        context.strokeStyle = rgba(colors.foregroundColor, alpha)
        context.beginPath()
  ...
  context.fillStyle = colors.foregroundColor
  context.shadowColor = colors.foregroundColor
...
export async function exportNodeGraphGif({ global, settings, colors, filename, viewScale: requestedViewScale = 1, onProgress }: Options) {
  const width = global.gifWidth || global.canvasWidth
  const height = global.gifHeight || global.canvasHeight
  ...
  const frameCount = Math.max(1, global.gifFrameCount)
  const fps = Math.max(1, global.gifFps)
  const delay = Math.round(1000 / fps)
  const gif = GIFEncoder()
  const nodes = distributeNodes(settings, width, height)
  const foreground = hexToRgb(colors.foregroundColor)
  const background = hexToRgb(colors.backgroundColor)
  let pulseT = 0

  for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
    const dt = 1 / fps
    pulseT = updateNodes(nodes, settings, width, height, dt, pulseT)
    nodes.forEach((node) => {
      node.rx += (node.x - node.rx) * 0.18
      node.ry += (node.y - node.ry) * 0.18
    })
    drawFrame(context, nodes, settings, colors, width, height, viewScale)
    const imageData = context.getImageData(0, 0, width, height)
    const palette = quantize(imageData.data, qualityToColors[global.gifQuality], { format: "rgb565" })
    snapColorsToPaletteLocal(palette, [
